...
then
        ...
        echo "Example train command : sh run.sh train <model_type> <dataset> <run_id> <GPU_id>"
        echo "Example perturb command : sh run.sh perturb <model_type> <dataset> <run_id> <GPU_id>"
        echo "Example perturb command : sh run.sh last_few_only <model_type> <dataset> <run_id> <GPU_id>"
        exit 0
else
        RUN_ID=$4
...
if [ -z "$5" ]
then
    echo "No GPU id provided. Supported : 0,1"
    echo "Example train command : sh run.sh train <model_type> <dataset> <run_id> <GPU_id>"
    echo "Example perturb command : sh run.sh perturb <model_type> <dataset> <run_id> <GPU_id>"
    echo "Example perturb command : sh run.sh last_few_only <model_type> <dataset> <run_id> <GPU_id>"
    exit 0
...
then
    ...
    TRAIN_MODEL_ARGS=$COMMON_ARGS" -vmt loss -eps 20 -veps 1 -stim 600 -bs 32 --optimizer adam --lr-scheduler invsqrt -lr 0.005 --dropout 0.3 --warmup-updates 4000"
elif [ "$2" = "s2s_bidir" ]
then
    echo "MODELTYPE: "$2
